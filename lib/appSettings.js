...
function isMissingTableError(error) {
  const message = String(error?.message || error || '')
  return /Could not find the table/i.test(message) || /relation .* does not exist/i.test(message) || (/schema cache/i.test(message) && /app_settings/i.test(message) && !isMissingColumnError(error))
}
