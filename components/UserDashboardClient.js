...
function ImageThumb({ item, fallback }) { const src = item?.thumb_url || item?.url || null; if (!src || !String(src).startsWith('http')) return <div className="content-thumb content-thumb-fallback">{fallback}</div>; return <img className="content-thumb" src={src} alt={item.caption || item.poi_title || fallback} loading="lazy" /> }
...
function DashboardItem({ href, title, subtitle, status = null, thumb = null, iconLabel = null, meta = null, url = '' }) {
  return (
    <Link className="dashboard-status-item" href={href}>
      {thumb ? thumb : <div className="content-thumb content-thumb-fallback">{iconLabel || '•'}</div>}
      <span>
        <strong>{title}</strong>
...
export default function UserDashboardClient() {
  ...
  const renderSectionItems = () => {
    if (section === 'favorites') return (lists.favorites || []).map((item) => (
      <DashboardItem key={item.id} href={item.pois?.slug ? `/poi/${item.pois.slug}` : '/account'} title={item.pois?.title || 'POI'} subtitle={formatDateTime(item.created_at)} iconLabel="♥" meta="Favorit" />
    ))
    if (section === 'reviews') return (lists.reviews || []).map((item) => (
      <DashboardItem key={item.id} href={item.pois?.slug ? `/poi/${item.pois.slug}#review-${item.id}` : '/account'} title={item.pois?.title || 'POI'} subtitle={item.review_text || formatDateTime(item.created_at)} iconLabel={<Star size={16} />} meta={`${item.rating}/5`} />
    ))
    if (section === 'replies') return (lists.replies || []).map((item) => (
      <DashboardItem key={item.id} href={item.poi_slug ? `/poi/${item.poi_slug}#reply-${item.id}` : '/account'} title={item.poi_title} subtitle={item.reply_text || formatDateTime(item.created_at)} iconLabel={<MessageSquareQuote size={16} />} meta={formatDateTime(item.created_at)} />
    ))
