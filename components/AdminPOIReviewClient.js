...
import { normalizeEditorialRecord, normalizeList, normalizeFamilyFriendly, normalizePoiRecord } from '@/lib/poiEditorial'
...
export default function AdminPOIReviewClient({ forcedPoiId = null }) {
  ...
  const notify = useCallback((message, type = 'info') => {
    setMessage(message || '')
    if (message) setToast({ message, type, duration: type === 'error' ? 7000 : 4500 })
  }, [])
  ...
  function applyDetail(detailData, mediaData) {
    const item = normalizePoiRecord(detailData.item || {})
    setSelected({
    ...
    const ed = normalizeEditorialRecord(detailData.editorial || {})
    setEditorial({
  ...
  async function loadMediaOnly(poiId) {
    if (!poiId) return
    const mediaData = await authFetchJson(`/api/admin/media?poi_id=${poiId}&t=${Date.now()}`)
    const nextMedia = mediaData.items || []
    setMedia(nextMedia)
    setActiveMediaId((prev) => prev && nextMedia.some((item) => item.id === prev) ? prev : (nextMedia[0]?.id || null))
  }

  async function loadDetail(poiId, options = {}) {
    if (!poiId) return
    if (dirtyRef.current && !options.force) {
      await loadMediaOnly(poiId)
      return
    }
    ...
    if (detailData.error) return notify(detailData.error, 'error')

    let mergedDetail = detailData
    const slug = detailData.item?.slug
    if (slug) {
      const publicData = await fetch(`/api/poi-public?slug=${encodeURIComponent(slug)}&t=${Date.now()}`, { cache: 'no-store' })
        .then((r) => r.json())
        .catch(() => null)
      if (publicData?.item) {
        const publicItem = normalizePoiRecord(publicData.item)
        const adminItem = normalizePoiRecord(detailData.item || {})
        const mergedItem = { ...adminItem }
        ;['state','city','address','short_description','description','opening_hours_text','price_info_text','hotels_nearby_text','website_url'].forEach((key) => {
          if (!String(mergedItem[key] || '').trim() && String(publicItem[key] || '').trim()) mergedItem[key] = publicItem[key]
        })
        mergedDetail = {
          ...detailData,
          item: mergedItem,
          editorial: detailData.editorial || publicData.editorial || null,
          links: detailData.links?.length ? detailData.links : (publicData.links || []),
          images: detailData.images?.length ? detailData.images : (publicData.images || []),
        }
      }
    }

    applyDetail(mergedDetail, mediaData)
  ...
  useEffect(() => {
    if (!queryPoiId) return
    function refreshWhenVisible() {
      if (document.visibilityState === 'visible') loadDetail(queryPoiId)
    }
  ...
  async function reverseGeocodeFill() {
    ...
    markDirty()
    notify(data.state || data.city || data.address ? 'Adressdaten wurden aus den Koordinaten übernommen.' : 'Für diese Koordinaten wurden keine Adressdaten gefunden.', data.state || data.city || data.address ? 'success' : 'info')
  }

  async function generateAI() {
    if (!selected?.id) return
    notify('KI generiert ...', 'info')
    ...
    if (!data.ok) return notify(data.error || 'KI-Aufruf fehlgeschlagen', 'error')
    ...
    notify('KI-Vorschlag geladen.', 'success')
  }
  ...
  function serializeEditorial() {
    return {
      ...
      family_friendly_json: normalizeFamilyFriendly(editorial.family_friendly_value === '' ? { reason: editorial.family_friendly_reason || '' } : { value: editorial.family_friendly_value === 'true', reason: editorial.family_friendly_reason || '' }),
  ...
  async function save(targetStatus = null) {
    ...
    if (poiResult.error) { setSaving(false); return notify(`Speichern fehlgeschlagen: ${poiResult.error}`, 'error') }
    const affiliateResult = await persistAffiliates()
    if (affiliateResult.error) { setSaving(false); return notify(`Affiliate-Speichern fehlgeschlagen: ${affiliateResult.error}`, 'error') }
    applyDetail({ ...poiResult, affiliate_settings: affiliateItems }, { items: media })
    dirtyRef.current = false
    setFormDirty(false)
    await loadDetail(selected.id, { force: true })
    setSaving(false)
    notify(targetStatus === 'published' ? 'Änderungen gespeichert und freigegeben.' : 'Änderungen gespeichert.', 'success')
  }

  function handleImagesUploaded(items = []) {
    if (!items.length) return
    setMedia((prev) => {
      const existing = new Set(prev.map((item) => item.id))
      const additions = items.filter((item) => item?.id && !existing.has(item.id))
      return additions.length ? [...additions, ...prev] : prev
    })
    setActiveMediaId((prev) => prev || items[0]?.id || null)
  }
  ...
  async function setMediaStatus(id, status) {
    ...
    notify(data.error || 'Bildstatus gespeichert', data.error ? 'error' : 'success')
